...
// Phone country codes mapped from ISO country codes
...
const evoaTripDetailsSchema = z.object({
...
type EVOATripDetailsFormData = z.infer<typeof evoaTripDetailsSchema>;

interface EVOATripDetailsStepProps {
  applicationId: string;
  onNext: () => void;
  onBack: () => void;
}

export default function EVOATripDetailsStep({ applicationId, onNext, onBack }: EVOATripDetailsStepProps) {
  ...
  // Try to parse existing phone into code + number
  ...
  const { register, handleSubmit, setError: setFormError, control, watch, setValue, formState: { errors } } = useForm<EVOATripDetailsFormData>({
    resolver: zodResolver(evoaTripDetailsSchema),
    defaultValues: {
  ...
  const onSubmit = useFormSubmit(setFormError, async (data: EVOATripDetailsFormData) => {
    ...
    // Save phone to first traveler and trip details to application
  ...
  return (
    <div>
      <div className="mb-8">
        ...
        <p className="text-gray-600">General details about your trip to Indonesia.</p>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
        ...
        {/* Flight Number - shown when flight is booked */}
        {hasFlightBooked === "true" && (
          <Input
            label="Flight number"
            placeholder="e.g. GA881"
