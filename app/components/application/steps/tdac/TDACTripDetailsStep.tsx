...
const tdacTripDetailsSchema = z.object({
...
type TDACTripDetailsFormData = z.infer<typeof tdacTripDetailsSchema>;

interface TDACTripDetailsStepProps {
  applicationId: string;
  onNext: () => void;
  onBack: () => void;
}

export default function TDACTripDetailsStep({ applicationId, onNext, onBack }: TDACTripDetailsStepProps) {
  ...
  const { register, handleSubmit, setError: setFormError, control, watch, setValue, formState: { errors } } = useForm<TDACTripDetailsFormData>({
    resolver: zodResolver(tdacTripDetailsSchema),
    defaultValues: {
  ...
  const onSubmit = useFormSubmit(setFormError, async (data: TDACTripDetailsFormData) => {
  ...
  return (
    <div>
      <div className="mb-8">
        ...
        <p className="text-gray-600">General details about your trip to Thailand.</p>
      </div>
